...
static void glnvg__checkError(GLNVGcontext* gl, const char* str)
{
  ...
  while ((err = QNANO_GLFWRAPPER glGetError()) != GL_NO_ERROR) {
...
static int glnvg__createShader(GLNVGshader* shader, const char* name, const char* header, const char* opts, const char* vshader, const char* fshader)
{
  ...
  QNANO_GLFWRAPPER glBindAttribLocation(prog, 0, "va_in");
  QNANO_GLFWRAPPER glBindAttribLocation(prog, 1, "vb_in");
  QNANO_GLFWRAPPER glBindAttribLocation(prog, 2, "normal_in");
...
static void glnvg__getUniforms(GLNVGshader* shader)
{
  ...
  shader->loc[GLNVG_LOC_TYPE] = QNANO_GLFWRAPPER glGetUniformLocation(shader->prog, "type");
  shader->loc[GLNVG_LOC_WINDINGTEX] = QNANO_GLFWRAPPER glGetUniformLocation(shader->prog, "windingTex");
  shader->loc[GLNVG_LOC_WINDINGYMIN] = QNANO_GLFWRAPPER glGetUniformLocation(shader->prog, "pathYMin");
  shader->loc[GLNVG_LOC_IMGPARAMS] = QNANO_GLFWRAPPER glGetUniformLocation(shader->prog, "imgParams");
  shader->loc[GLNVG_LOC_WINDINGIMG] = QNANO_GLFWRAPPER glGetUniformLocation(shader->prog, "windingImg");
...
static int glnvg__renderCreateTexture(void* uptr, int type, int w, int h, int imageFlags, const void* data)
{
  ...
  if (type == NVG_TEXTURE_RGBA) {
    ...
    QNANO_GLFWRAPPER glTexImage2D(GL_TEXTURE_2D, 0, internalfmt, w, h, 0, GL_RGBA, GL_UNSIGNED_BYTE, data);
  } else if (type == NVG_TEXTURE_FLOAT) {
    QNANO_GLFWRAPPER glTexImage2D(GL_TEXTURE_2D, 0, GL_R32F, w, h, 0, GL_RED, GL_FLOAT, data);
  } else {
  ...
  QNANO_GLFWRAPPER glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, minfilt);
  QNANO_GLFWRAPPER glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, magfilt);
  QNANO_GLFWRAPPER glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, wrapx);
  QNANO_GLFWRAPPER glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, wrapy);
...
static int glnvg__renderUpdateTexture(void* uptr, int image, int x, int y, int w, int h, const void* data)
{
  ...
  if (tex->type == NVG_TEXTURE_RGBA)
    ...
    QNANO_GLFWRAPPER glTexSubImage2D(GL_TEXTURE_2D, 0, x,y, w,h, GL_RED, GL_FLOAT, data);
  else
#if defined(NANOVG_GLES2) || defined(NANOVG_GL2)
...
static void glnvg__renderViewport(void* uptr, float width, float height, float devicePixelRatio)
{
  ...
  if (gl->view[0] != width || gl->view[1] != height) {
    if(gl->texWinding > 0)
      QNANO_GLFWRAPPER glDeleteTextures(1, &gl->texWinding);
    if(gl->texColor > 0)
      QNANO_GLFWRAPPER glDeleteTextures(1, &gl->texColor);
    gl->texWinding = 0;
    gl->texColor = 0;
  }
  gl->view[0] = width;
...
// Also: could we use glBlendFunci, QNANO_GLFWRAPPER glColorMaski for FB swap?
static void glnvg__fill(GLNVGcontext* gl, GLNVGcall* call)
{
  ...
  if (gl->renderMethod == NVGL_RENDER_FB_SWAP) {
    ...
    QNANO_GLFWRAPPER glUniform1i(gl->shader.loc[GLNVG_LOC_TYPE], NSVG_SHADER_WINDINGCLEAR);
  ...
  QNANO_GLFWRAPPER glUniform1i(gl->shader.loc[GLNVG_LOC_TYPE], NSVG_SHADER_WINDINGCALC);
  for (i = 0; i < npaths; i++) {
    glUniform1f(gl->shader.loc[GLNVG_LOC_WINDINGYMIN], paths[i].yMin < 0 ? 0 : paths[i].yMin);
    QNANO_GLFWRAPPER glDrawArrays(GL_TRIANGLES, paths[i].fillOffset, paths[i].fillCount);
  }
  glnvg__checkError(gl, "fill: accumulate winding");

  // read from winding texture to fill
  QNANO_GLFWRAPPER glUniform1i(gl->shader.loc[GLNVG_LOC_TYPE], call->fragType);
  // restore blend func
  if (gl->renderMethod != NVGL_RENDER_FB_FETCH)
  ...
  if (gl->renderMethod == NVGL_RENDER_FB_SWAP) {
    ...
    QNANO_GLFWRAPPER glActiveTexture(GL_TEXTURE0 + 1);
    QNANO_GLFWRAPPER glBindTexture(GL_TEXTURE_2D, gl->texWinding);
  }
...
static void glnvg__convexFill(GLNVGcontext* gl, GLNVGcall* call)
{
  ...
  //for (i = 0; i < npaths; i++)
  QNANO_GLFWRAPPER glDrawArrays(GL_TRIANGLE_FAN, call->triangleOffset, call->triangleCount);
...
static void glnvg__renderFlush(void* uptr)
{
  ...
  QNANO_GLFWRAPPER glGetIntegerv(GL_DRAW_FRAMEBUFFER_BINDING, (GLint*)&gl->fbUser);
  // (re)create winding accum texture
  if (gl->texWinding == 0) {
    QNANO_GLFWRAPPER glGenTextures(1, &gl->texWinding);
    QNANO_GLFWRAPPER glActiveTexture(GL_TEXTURE0 + 1);
    QNANO_GLFWRAPPER glBindTexture(GL_TEXTURE_2D, gl->texWinding);
    // w/ gl_LastFragData or glTextureBarrier, we could use a single RGBA16F texture for color and winding
    //QNANO_GLFWRAPPER glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA16F, width, height, 0, GL_RGBA, GL_HALF_FLOAT, NULL);
    if (gl->renderMethod == NVGL_RENDER_IMG_ATOMIC)
#ifdef GL_ES_VERSION_3_0
      glTexStorage2D(GL_TEXTURE_2D, 1, GL_R32I, gl->view[0], gl->view[1]);
#else
      QNANO_GLFWRAPPER glTexImage2D(GL_TEXTURE_2D, 0, GL_R32I, gl->view[0], gl->view[1], 0, GL_RED_INTEGER, GL_INT, NULL);
#endif
    else if(gl->renderMethod == NVGL_RENDER_FB_FETCH)
      QNANO_GLFWRAPPER glTexImage2D(GL_TEXTURE_2D, 0, GL_R32I, gl->view[0], gl->view[1], 0, GL_RED_INTEGER, GL_INT, NULL);
    else // FB swap -- must use float since blending doesn't work with ints
      QNANO_GLFWRAPPER glTexImage2D(GL_TEXTURE_2D, 0, GL_R32F, gl->view[0], gl->view[1], 0, GL_RED, GL_FLOAT, NULL);
  ...
  if (gl->renderMethod == NVGL_RENDER_FB_FETCH)
    QNANO_GLFWRAPPER glDisable(GL_BLEND);
  else
  ...
  // 0 = va_in, 1 = vb_in, 2 = normal_in as set by QNANO_GLFWRAPPER glBindAttribLocation in glnvg__createShader
  QNANO_GLFWRAPPER glEnableVertexAttribArray(0);
  QNANO_GLFWRAPPER glEnableVertexAttribArray(1);
  QNANO_GLFWRAPPER glEnableVertexAttribArray(2);
  QNANO_GLFWRAPPER glVertexAttribPointer(0, 2, GL_FLOAT, GL_FALSE, sizeof(NVGvertex), (const GLvoid*)(size_t)0);
  QNANO_GLFWRAPPER glVertexAttribPointer(1, 2, GL_FLOAT, GL_FALSE, sizeof(NVGvertex), (const GLvoid*)(0 + 2*sizeof(float)));  //offsetof(NVGVertex, x1)
  QNANO_GLFWRAPPER glVertexAttribPointer(2, 2, GL_FLOAT, GL_FALSE, sizeof(NVGvertex), (const GLvoid*)(0 + 4*sizeof(float)));  //offsetof(NVGVertex, u);

  // Set view and texture locations just once per frame.
  glUniform2fv(gl->shader.loc[GLNVG_LOC_VIEWSIZE], 1, gl->view);
  QNANO_GLFWRAPPER glUniform1i(gl->shader.loc[GLNVG_LOC_TEX], 0);
  if (gl->renderMethod == NVGL_RENDER_FB_SWAP)
    QNANO_GLFWRAPPER glUniform1i(gl->shader.loc[GLNVG_LOC_WINDINGTEX], 1);
#ifndef NANOVG_GLES3
  // in GLES, image unit location can't be set w/ glUniform, only queried - so we use binding = 0 in shader
  else if(gl->renderMethod == NVGL_RENDER_IMG_ATOMIC)
    QNANO_GLFWRAPPER glUniform1i(gl->shader.loc[GLNVG_LOC_WINDINGIMG], 0);
#endif
  ...
  QNANO_GLFWRAPPER glDisableVertexAttribArray(2);
#if defined NANOVG_GL3
